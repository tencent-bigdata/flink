...
{% endhighlight %}
    ...
    <tr>
      <td>
        {% highlight text %}
JSON_VALUE(string1, string2)
{% endhighlight %}
      </td>
      <td>
        <p>Extract the value of the specified json path from the JSON string, and both invalid JSON and null return null uniformly.</p>
      </td>
    </tr>
...
{% endhighlight %}
      ...
      <td>
        <p> Returns the absolute value of hashCode() of the string.Returns NULL If the String is NULL.</p>
      </td>
    </tr>
    ...
    <tr>
      <td>
        {% highlight java %}
PARSE_URL(urlString, partName[, key])
{% endhighlight %}
      </td>
      <td>
        <p> Parse the url to get the value of partName, such as partName= 'QUERY', and get the
        value of url parameter key, partName can be HOST, PATH, QUERY, REF, PROTOCOL, FILE,
        AUTHORITY, USERINFO. </p>
      </td>
    </tr>
...
{% endhighlight %}
    ...
    <tr>
      <td>
        {% highlight text %}
KEYVALUE(string, string1, string2, key)
{% endhighlight %}
      </td>
      <td>
        <p>Parse the key-value pair in the string. Returns the corresponding value of the key. Returns NULL if the value of the key does not exist or an exception </p>
      </td>
    </tr>
...
UNIX_TIMESTAMP(dateSTRING, formatSTRING)
{% endhighlight %}
      </td>
      <td>
        <p>Two parameters are optional, no parameter returns the current timestamp. the first
           parameter is the string types of time, the second parameter is the format of the time, the default for MM - dd yyyy - hh: MM: ss, the return value is the first parameter into a long integer timestamp, if a parameter is null or parsing error, return -1.</p>
      </td>
    </tr>
    ...
    <tr>
      <td>
        {% highlight scala %}
FROM_UNIXTIME(unixtime[, formatString])
{% endhighlight %}
      </td>
      <td>
        <p>The first parameter, unixtime, is a long integer and is a timestamp in seconds. The
        second parameter, format, is optional and is the date format. By default, it is yyyy-mm-dd
        HH: MM :ss.The return value is a DATE</p>
      </td>
    </tr>
...
{% endhighlight %}
      ...
      <td>
        <p>Returns a timestamp of the current time zone time (in seconds). Or accepts an integer
        parameter as an offset (in seconds) and returns the sum of the offset and current timestamp.</p>
      </td>
    </tr>
